...
if [[ -z "$KAFKA_ADVERTISED_PORT" ]] && [[ -z "$KAFKA_LISTENERS" ]] && [[ -z "$KAFKA_ADVERTISED_LISTENERS" ]]; then
    echo "ERROR: missing mandatory config: KAFKA_ADVERTISED_PORT, KAFKA_LISTENERS, or KAFKA_ADVERTISED_LISTENERS"
    exit 1
fi
