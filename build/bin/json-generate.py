...
# FIXME: allow a user to specify a schema

def usage():
    res = argparse.ArgumentParser(prog='json-generate.py', description='Generate a json document from any variables provided as arguments', add_help=True)
    res.add_argument('pairs', nargs='*', type=str, help='key=value pairs to define document')
    res.add_argument('-l', '--list', action='store_true', help='treat the arguments as a list instead of key-value pairs')
    res.add_argument('-e', '--environment', action='store_true', help='use the pairs defined in the environment')
    res.add_argument('-p', '--path', type=str, default='', help='the path to the branch to write pairs to')
